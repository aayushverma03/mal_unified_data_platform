  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage{array}
        \usepackage{etoolbox}
        \definecolor{tblHdrBg}{HTML}{0F172A}
        \definecolor{tblHdrText}{HTML}{FFFFFF}
        ...
        \setlist{nosep}
        % Apply zebra striping inside pandoc's longtable environment.
        % Row 1 is the header (kept white so the booktabs rules read clean);
        % row 2 onwards alternates F1F5F9 / white.
        \AtBeginEnvironment{longtable}{\rowcolors{2}{tblRowAlt}{white}}
        % High and Medium priority badges for the gap table.
...
|:----|:-------------|:---------------------|:--------:|
| Orchestration | One Python process | Dagster (asset-based, SQL sensors, lineage UI) | `\hi`{=latex}`<span class="prio-high">High</span>`{=html} |
| Ingestion | CSV files in `data/raw/` | Debezium and Kafka for CDC, 30-second freshness SLO | `\hi`{=latex}`<span class="prio-high">High</span>`{=html} |
| Customer identity | `customer_id` string trusted from each squad | Identity service deduplicating across phone, email, national ID | `\hi`{=latex}`<span class="prio-high">High</span>`{=html} |
...
| Schema registry | Pydantic models in the repo | Confluent Schema Registry or Git-based registry with evolution rules | `\md`{=latex}`<span class="prio-med">Medium</span>`{=html} |
| Multi-region DR | Single local Parquet directory | UAE-resident primary, second region for DR, CBUAE residency rules | `\md`{=latex}`<span class="prio-med">Medium</span>`{=html} |
| Backfill and replay | Re-run `make seed` and `make run` | Replay-from-Kafka with idempotent writes, backfill jobs in Dagster | `\md`{=latex}`<span class="prio-med">Medium</span>`{=html} |
| Dashboards | Static GitHub Pages report, Streamlit (D3) | Looker or Superset on the canonical store, with squad-specific saved views | `\md`{=latex}`<span class="prio-med">Medium</span>`{=html} |
